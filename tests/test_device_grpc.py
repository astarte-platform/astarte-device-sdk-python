...
class TestMyAbstract(unittest.TestCase):
    ...
    def test_devicegrpc_remove_interface_connected_device(
        self, mock_remove_interface, mock_json_dumps, mock_interfaces_name
    ):
        node_uuid = "node uuid"
        device = DeviceGrpc(
            "server address",
        ...
        mock_json_dumps.assert_called_once_with(interface_name)
        mock_interfaces_name.assert_called_once_with(
            names=[mock_json_dumps.return_value.encode.return_value]
        )
